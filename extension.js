...
function activate() {
    ...
    vscode.commands.registerCommand('extension.getAccounts', async () => {
        const accounts = await vscode.authentication.getAccounts('microsoft');
        outputChannel.info(`Found ${accounts.length} accounts`);
    });

    vscode.commands.registerCommand('extension.getSessionSilent', async () => {
        const session = await vscode.authentication.getSession('microsoft', [], { silent: true });
        if (session) {
            outputChannel.info(`Found session for account: ${session.account.label}`);
        } else {
